...
# for OpenAPI schema/ui generation
from weaver import WEAVER_MODULE_DIR, __meta__  # isort:skip # noqa: E402 # pylint: disable=C0413
from weaver.wps_restapi.api import get_swagger_json  # isort:skip # noqa: E402
from mako.template import Template  # isort:skip # noqa: E402
...
doc_redirect_ignores = [
    ...
    re.compile(r"index.*"),
    re.compile(r"api.*"),  # corresponding 'api.html' created manually below
]
...
api_html_path = os.path.join(DOC_BLD_ROOT, "html")
api_spec_file = os.path.join(api_html_path, "api.json")
api_spec_json = get_swagger_json(http_host="example", http_scheme="https")
api_spec_json_js = api_spec_file + ".js"
os.makedirs(api_html_path, exist_ok=True)
with open(api_spec_file, "w") as f:
    json.dump(api_spec_json, f)
# define a JS script that contains a variable with the JSON OpenAPI specification
# this is needed in order for the swagger-ui to load it
with open(api_spec_json_js, "w") as f:
    text = json.dumps(api_spec_json)
    f.write("var json_spec = " + text)
api_data_mako = {
    "api_title": "{} API".format(__meta__.__title__),
    "api_version": __meta__.__version__,
    "api_loader": "spec: json_spec,",  # variable found within below js script
    "api_scripts": "<script src=\"api.json.js\"></script>"
}
# generate the file as 'openapi.html' because 'api.html' gets generated by the documentation RST
# file 'api.rst' is referenced in the RST TOC in order to generate the link
