...
- | ``weaver.wps_output_bucket = <s3-bucket>``
  ...
  |
  | The S3 bucket needs to be accessible by the application, which can require from the user to specify credentials by
    one of the `supported methodologies <aws-credentials>`_ to provide necessary role or permissions. See also reference
    `AWS Configuration <aws-config>`_ options that will be considered when working with S3 buckets.
