...
def _get_username(request):
    ...
    if 'weaver.username' in settings:
        username = settings['weaver.username']
        if username:
...
def _get_password(request):
    settings = request.registry.settings
    if 'weaver.password' in settings:
        password = settings['weaver.password']
        if password:
...
def auth_activated(registry):
    ...
    username = settings.get('weaver.username')
    if username:
