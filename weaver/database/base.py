...
def get_database_factory_type(registry):
    ...
    """
    Obtains `weaver.db_factory` from `config.registry.settings` and validates it.
    :returns: A string representing the configured database factory. (default: 'mongodb')
    """
    from weaver.database.postgres import PostgresDatabase
    from weaver.database.mongodb import MongoDatabase
    from weaver.database.memory import MemoryDatabase

    settings = registry.settings
    db_factory = settings.get('weaver.db_factory', MongoDatabase.type)
    if db_factory in frozenset([PostgresDatabase.type, MongoDatabase.type, MemoryDatabase.type]):
        return db_factory
    raise NotImplementedError("Unknown settings `weaver.db_factory` == `{}`.".format(str(db_factory)))


def get_database_factory(registry):
    # type: (Registry) -> DatabaseInterface
    """Get the database factory corresponding to setting `weaver.db_factory`."""
    from weaver.database.postgres import PostgresDatabase
    from weaver.database.mongodb import MongoDatabase
    from weaver.database.memory import MemoryDatabase
