...
from weaver.processes.convert import wps2json_io
...
from weaver.wps.utils import check_wps_status, get_wps_path, load_pywps_config
...
class WorkerService(Service):
    ...
    def execute(self, identifier, wps_request, uuid):
        # type: (AnyStr, WPSRequest, AnyStr) -> WPSResponse
        """
        ...
        data = {
            "inputs": [wps2json_io(i) for i in wps_request.inputs],
            "outputs": [wps2json_io(o) for o in wps_request.outputs],
            "response": EXECUTE_RESPONSE_DOCUMENT,
            "mode": EXECUTE_MODE_ASYNC,
        }
        for output in data["outputs"]:
            output["transmissionMode"] = EXECUTE_TRANSMISSION_MODE_REFERENCE
        ...
        # when called by the WSGI app, '__call__' will generate the XML from 'doc',
        # which itself is generated by template substitution from the data obtained by 'json'
